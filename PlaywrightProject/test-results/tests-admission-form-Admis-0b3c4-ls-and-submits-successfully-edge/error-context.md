...
- Name: tests\admission-form.spec.ts >> Admission form fills all mandatory details and submits successfully
- Location: tests\admission-form.spec.ts:3:5
...
```ts
  ...
  2  |
  3  | test('Admission form fills all mandatory details and submits successfully', async ({ page }) => {
> 4  |   await page.goto('http://localhost:3000', { waitUntil: 'domcontentloaded', timeout: 120000 });
  ...
  8  |   await page.locator('#email').fill('k.b.pavithra@capgemini.com');
