...
- Name: tests\admission-form-yahoo.spec.ts >> Admission form submits successfully with yahoo.com email
- Location: tests\admission-form-yahoo.spec.ts:3:5
...
```ts
  ...
  2  |
  3  | test('Admission form submits successfully with yahoo.com email', async ({ page }) => {
> 4  |   await page.goto('http://localhost:3000', { waitUntil: 'domcontentloaded', timeout: 120000 });
  ...
  8  |   await page.locator('#email').fill('ravi.kumar@yahoo.com');
